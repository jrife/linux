...
static int cmp_addr_log(const char *msg, int af,
...
static int test_kernel_bind(const struct sock_addr_test *test)
{
	...
	if (cmp_addr_log("kernel_bind() modified the address parameter ", 
			 test->domain, &requested_addr, &results.addr, 1))
		goto err;

	if (cmp_addr_log("kernel_getsockname() did not return the expected "
			 "address ",
			 test->domain, &expected_addr, &results.sock_name, 1))
		goto err;
...
static int test_kernel_connect(const struct sock_addr_test *test)
{
	socklen_t addr_len = sizeof(struct sockaddr_storage);
	...
	if (run_mod_test(test, CONNECT, &results))
		goto err;
	
	if (!results.success)
		goto err;
	
	if (cmp_addr_log("kernel_connect() modified the address parameter ",
			 test->domain, &requested_addr, &results.addr, 1))
		goto err;
	
	if (cmp_addr_log("kernel_getsockname() did not return the expected "
			 "address ", test->domain, &expected_src_addr,
			 &results.sock_name, 0))
		goto err;

	if (cmp_addr_log("kernel_getpeername() did not return the expected "
			 "address ", test->domain, &expected_addr,
			 &results.peer_name, 1))
		goto err;
...
static int test_kernel_sendmsg(const struct sock_addr_test *test)
{
	socklen_t addr_len = sizeof(struct sockaddr_storage);
	struct sock_addr_testmod_results results;
	struct sockaddr_storage expected_addr;
	...
	if (run_mod_test(test, SENDMSG, &results))
		goto err;

	if (!results.success)
		goto err;
	
	if (cmp_addr_log("kernel_sendmsg() modified msg_name ", test->domain,
			 &sendmsg_addr, &results.addr, 1))
		goto err;
	...
	if (cmp_addr_log("unexpected source address ", test->domain,
			 &recvmsg_addr, &expected_addr, 0))
		goto err;
...
static int run_test_case(int cgfd, const struct sock_addr_test *test)
{
	...
	if (bpf_prog_attach(progfd, cgfd, test->attach_type,
	...
	case BPF_CGROUP_INET6_BIND:
		err = test_kernel_bind(test);
		break;
	case BPF_CGROUP_INET4_CONNECT:
	case BPF_CGROUP_INET6_CONNECT:
		err = test_kernel_connect(test);
		break;
	case BPF_CGROUP_UDP4_SENDMSG:
	case BPF_CGROUP_UDP6_SENDMSG:
		err = test_kernel_sendmsg(test);
		break;
	default:
		goto err;
	}
